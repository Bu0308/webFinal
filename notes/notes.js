...
function showPasswordModal(callback) {
    const modal = document.getElementById("password-modal-1");
    const input = document.getElementById("password-input-1");
    ...
    try {
        ...
        if (!isLocked) {
            ...
            showPasswordModal(async (password) => {
                await updatePassword(noteId, email, password);
            });
        } else {
            // Nếu đã khóa, gửi request để bỏ mật khẩu (password rỗng)
            await updatePassword(noteId, email, '');
        }
    } catch (err) {
        console.error('Error setting/removing password:', err);
        alert('Lỗi khi cập nhật mật khẩu. Vui lòng thử lại.');
    }
}

async function updatePassword(noteId, email, password) {
    try {
        ...
        notes[noteIndex].isLocked = updatedNote.isLocked;
        renderNotes();
        alert(updatedNote.message);
    } catch (err) {
        console.error('Error in updatePassword:', err);
        alert('Lỗi khi gửi yêu cầu cập nhật mật khẩu.');
    }
}
